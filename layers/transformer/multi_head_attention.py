...
class MultiHeadAttention(AbsPosAttentionBase):
    def __init__(
        ...
        # I switched these two around. If input_size is not none, the LOOKUP / thing that is attended to should be input_size
        # https://github.com/huggingface/diffusers/blob/4125756e88e82370c197fecf28e9f0b4d7eee6c3/src/diffusers/models/cross_attention.py
        # See also line 81,82 in this file
        self.data_to_kv = torch.nn.Linear(
            state_size if input_size is None else input_size,
            2 * n_heads * self.projection_size,
            bias=False,
        )
    ...
    def forward(
        ...
        if value_zero_index is not None:
            v[:, value_zero_index, :] = 0

        data, scores = self.merged_attention(
            curr_state.shape[0], q.shape[1], mask, q, k, v
        )
        if need_weights:
            return data, scores.mean(1) if average_attn_weights else scores
        else:
            return data
